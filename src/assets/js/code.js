 ...
 */
 function fetchMessages() {
  if (messenger != 0 && previousPageNumber != -1) {
    $.ajax({
      url: url + "/fetchMessages",
      method: "POST",
      data: { _token: access_token, id: to_id, type: message_type, previous_page_number: previousPageNumber },
      dataType: "JSON",
      success: (data) => {
        ...
        if(!previousPageNumber) {
          ...
          $('.app-scroll')
            .stop()
            .animate({
              scrollTop: $('.app-scroll')[0].scrollHeight - 20,
            });
        }
