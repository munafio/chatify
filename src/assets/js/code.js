 ...
 *-------------------------------------------------------------
 */
function sendMessage(isVoiceMessage = false, audioBlob = null, duration = null, fileName = "voice-message.webm") {
 temporaryMsgId += 1;
 ...
 $.ajax({
   ...
   beforeSend: () => {
     ...
     const tempMessage = isVoiceMessage
       ? `${loadingSVG("18px")} Sending voice message`
       : `${sanitizeInput(inputValue)}${hasFile ? "\n" + loadingSVG("28px") : ""}`;

     messagesContainer
       .find(".messages")
       .append(sendTempMessageCard(tempMessage, tempID));
   ...
   success: (data) => {
     if (data.error?.status || data.error > 0) {
       errorMessageCard(tempID);
       console.error(data.error?.message || data.error_msg);
     } else {
       ...
       const actualMessageIdMatch = message.match(/data-id="([a-z0-9\-]+)"/);
   ...
   $.ajax({
     ...
     success: (data) => {
       ...
       if (messagesPage == 1) {
         ...
         // Update voice messages and initialize WaveSurfer players.
         if (typeof renderWavesurfers === "function") {
           renderWavesurfers();
         }
